...
bitflags!(
    ...
    pub struct ImGuiBackendFlags: c_int {
        ...
        const PlatformHasViewports = 1 << 10;
        const HasMouseHoveredViewports = 1 << 11;
        const RendererHasViewports = 1 << 12;
    }
);
...
bitflags!(
    ...
    pub struct ImGuiConfigFlags: c_int {
        ...
        const DockingEnable = 1 << 6;
        const ViewportsEnable = 1 << 10;
        const DpiEnableScaleViewports = 1 << 14;
        const DpiEnableScaleFonts = 1 << 15;
...
bitflags!(
    ...
    #[repr(C)]
    pub struct ImGuiDockNodeFlags: c_int {
        const KeepAliveOnly = 1;
        const NoSplit = 1 << 1;
        const NoDockingInCentralNode = 1 << 3;
        const NoResize = 1 << 5;
        const PassthruDockspace = 1 << 6;
    }
);
...
bitflags!(
    /// Flags for tab bars
    #[repr(C)]
    pub struct ImGuiTabBarFlags: c_int {
        const Reorderable = 1;
        const AutoSelectNewTabs = 1 << 1;
        const NoCloseWithMiddleMouseButton = 1 << 2;
        const NoTabListPopupButton = 1 << 3;
        const NoTabListScrollingButtons = 1 << 4;
        const NoTooltip = 1 << 5;
        const FittingPolicyResizeDown = 1 << 6;
        const FittingPolicyScroll = 1 << 7;
        const FittingPolicyMask = ImGuiTabBarFlags::FittingPolicyResizeDown.bits | ImGuiTabBarFlags::FittingPolicyScroll.bits;
        const FittingPolicyDefault = ImGuiTabBarFlags::FittingPolicyResizeDown.bits;
    }
);
...
bitflags!(
    /// Flags for tab items
    #[repr(C)]
    pub struct ImGuiTabItemFlags: c_int {
        const UnsavedDocument = 1;
        const SetSelected = 1 << 1;
        const NoCloseWithMiddleMouseButton = 1 << 2;
        const NoPushId = 1 << 3;
    }
);
...
bitflags!(
    /// Viewport flags
    #[repr(C)]
    pub struct ImGuiViewportFlags: c_int {
        const NoDecoration = 1;
        const NoTaskBarIcon = 1 << 1;
        const NoFocusOnAppearing = 1 << 2;
        const NoInputs = 1 << 3;
        const NoRendererClear = 1 << 4;
        const TopMost = 1 << 5;
    }
);
...
bitflags!(
    ...
    pub struct ImGuiWindowFlags: c_int {
        ...
        const UnsavedDocument = 1 << 20;
        const NoDocking = 1 << 21;
        ...
        const NavFlattend = 1 << 23;
        const ChildWindow = 1 << 24;
        const Tooltip = 1 << 25;
        const Popup = 1 << 26;
        const Modal = 1 << 27;
        const ChildMenu = 1 << 28;
        const DockNodeHost = 1 << 29;
    }
);
